...
class ImprovedFlowerClient(fl.client.NumPyClient):
    def __init__(self, model, X_train, y_train, X_test, y_test, client_id):
        ...
        logger.info(f"Improved Client {client_id} initialized on device: {self.device}")
...
def run_client():
    parser = argparse.ArgumentParser(description='Improved Federated Learning Client for Diabetes Prediction')
    ...
    try:
        logger.info("=" * 60)
        logger.info(f"Starting IMPROVED Federated Learning Client (ID: {args.client_id})")
        logger.info("=" * 60)
        
        # Load and preprocess the dataset with Non-IID split
        X_train, X_test, y_train, y_test, scaler = load_diabetes_data(
        ...
        client = ImprovedFlowerClient(model, X_train, y_train, X_test, y_test, args.client_id)
